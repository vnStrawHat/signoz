...
func TestFuncCutOffMin(t *testing.T) {
	...
	for _, tt := range tests {
		...
		for i, res := range newResult {
			for j, series := range res.Series {
				for k, point := range series.Points {

					if math.IsNaN(tt.want[i].Series[j].Points[k].Value) {
						if !math.IsNaN(point.Value) {
							t.Errorf("funcCutOffMin() = %v, want %v", point.Value, tt.want[i].Series[j].Points[k].Value)
						}
						continue
					}

					if point.Value != tt.want[i].Series[j].Points[k].Value {
						t.Errorf("funcCutOffMin() = %v, want %v", point.Value, tt.want[i].Series[j].Points[k].Value)
					}
				}
			}
		...
		for i, res := range newResult {
			for j, series := range res.Series {
				for k, point := range series.Points {
					...
					if math.IsNaN(tt.want[i].Series[j].Points[k].Value) {
						if !math.IsNaN(point.Value) {
							t.Errorf("funcCutOffMax() = %v, want %v", point.Value, tt.want[i].Series[j].Points[k].Value)
						}
						continue
					}
